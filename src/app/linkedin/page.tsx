...
import { LinkedInPost } from '@/components/templates/LinkedInPost';
import { LinkedInEditorControls } from '@/components/editor/LinkedInEditorControls';
...
export default function LinkedInCreatePage() {
    ...
    const {
        ...
        linkedinDisplayName,
        linkedinUsername,
        linkedinCustomBadgeUrl,
        linkedinShowCustomBadge,
        linkedinContent,
        linkedinPostImageUrl,
        linkedinShowImage,
        linkedinPostImage2Url,
        linkedinShowImage2
    } = usePostStore();
    ...
    const handleReset = () => {
        updateField('linkedinDisplayName', '');
        updateField('linkedinUsername', '');
        updateField('linkedinContent', '');
        updateField('linkedinTimestamp', '');
        updateField('linkedinAvatarUrl', null);
        updateField('linkedinPostImageUrl', null);
        updateField('linkedinShowImage', false);
        updateField('linkedinShowImage2', false);
        updateField('linkedinIsVerified', false);
        updateField('linkedinFollowText', '');
        updateField('linkedinConnectionDegree', '');

        // Reset Stats
        updateStat('likes', '10');
        updateStat('replies', '10');
        updateStat('retweets', '10');
    };
    ...
    const handleDownload = async () => {
        ...
        if (linkedinShowCustomBadge && (!linkedinCustomBadgeUrl || linkedinCustomBadgeUrl.length === 0)) {
        ...
        if (linkedinShowImage && (!linkedinPostImageUrl || linkedinPostImageUrl.length === 0)) {
            toast.error("Please upload an image or disable the image option!");
            return;
        }

        // Check for Second Image
        if (linkedinShowImage2 && (!linkedinPostImage2Url || linkedinPostImage2Url.length === 0)) {
            toast.error("Please upload a second image or disable the option!");
            return;
        }

        const node = document.getElementById('linkedin-canvas');
        if (!node) return;
        ...
        try {
            ...
            link.download = 'linkedin-mockup.png';
            link.href = dataUrl;
            link.click();
                ...
                <div className="flex-1 h-full relative overflow-y-auto scrollbar-hide">
                    ...
                    <div className="min-h-full flex flex-col items-center p-2 sm:p-6 lg:p-12 relative z-10 py-8 lg:justify-center">
                        ...
                        <div className="relative w-full flex justify-center origin-top transform-gpu scale-[0.5] sm:scale-[0.6] md:scale-[0.85] xl:scale-100 transition-transform duration-300">
                            <LinkedInPost id="linkedin-canvas" />
                        </div>
                        ...
                        <div className="flex justify-center w-full z-50 pointer-events-none">
                            <div className={cn(
                                ...
                                <button onClick={handleReset} className="p-1.5 sm:p-2 rounded-full hover:bg-red-500/10 hover:text-red-500 transition-colors" title="Reset Canvas">
